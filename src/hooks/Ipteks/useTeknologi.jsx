...
import { getTeknologi, createTeknologi, deleteTeknologi, downloadTeknologiTemplate, importTeknologi } from '../../service/AnalisisKonsideran/Ipteks';
...
export const useTeknologi = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await getTeknologi(user?.prodiId);
  ...
  const handleExportTemplateTeknologi = async () => {
    try {
      ...
      message.error("Gagal mengunduh template: " + (error.response?.data?.message || error.message));
    }
  };
