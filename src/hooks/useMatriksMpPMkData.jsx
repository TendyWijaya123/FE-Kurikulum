...
export const useMatriksMpPMkData = () => {
	...
	useEffect(() => {
		const fetchMatriksPengetahuanMp = async (prodiId = null) => {
			setLoading(true);
			try {
				const data = await getMatrixMpPMK(prodiId);
	...
	const handleProdiChange = async (value) => {
		...
		setLoading(true);
		try {
			const data = await getMatrixMpPMK(value);
			setPengetahuan(data.pengetahuans);
			setMp(data.mps);
			setMatrix(data.matrix);
		} catch (error) {
	...
	const handleDropdownChange = (
		...
		if (updatedMatrix[rowIndex][colIndex].enabled) {
			...
			message.warning(
				"Sel ini tidak dapat diubah karena relasi MP-P tidak tersedia."
			);
		}
	};
