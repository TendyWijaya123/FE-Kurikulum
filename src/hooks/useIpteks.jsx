...
export const useIpteks = () => {
	...
	const handleDelete = async (index) => {
		setLoading(true);
		try {
			const updatedIpteksData = [...ipteksData];
			const itemToDelete = updatedIpteksData[index];

			if (itemToDelete?.id) {
				await deleteIpteks(itemToDelete.id);
			}

			updatedIpteksData.splice(index, 1);
			setIpteksData(updatedIpteksData);

			message.success("Data berhasil dihapus");
		} catch (error) {
			message.error(`Gagal menghapus data: ${error.message || error}`);
		} finally {
			setLoading(false);
		}
	};
