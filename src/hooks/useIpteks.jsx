...
import { getIpteks, deleteIpteks, createIpteks, updateIpteks } from '../service/api';
...
export const useIpteks = () => {
  ...
  const fetchIpteks = async () => {
    ...
    try {
      ...
      const maxLength = Math.max(pengetahuan.length, teknologi.length, seni.length);
  ...
  const handleCreate = async (values) => {
    try {
        ...
        if (values.pengetahuan) {
            updates.push(createIpteks('pengetahuan', {
                ilmu_pengetahuan: values.pengetahuan
            }));
        }

        if (values.teknologi) {
            updates.push(createIpteks('teknologi', {
                teknologi: values.teknologi
            }));
        }

        if (values.seni) {
            updates.push(createIpteks('seni', {
                seni: values.seni
            }));
        }
...
const handleUpdate = async (row, values) => {
  try {
      ...
      if (row.pengetahuan) {
          updates.push(updateIpteks('pengetahuan', row.pengetahuan.id, {
              ilmu_pengetahuan: values.pengetahuan
          }));
      }

      if (row.teknologi) {
          updates.push(updateIpteks('teknologi', row.teknologi.id, {
              teknologi: values.teknologi
          }));
      }

      if (row.seni) {
          updates.push(updateIpteks('seni', row.seni.id, {
              seni: values.seni
          }));
      }
