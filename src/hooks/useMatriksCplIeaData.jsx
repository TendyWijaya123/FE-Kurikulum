...
import {
	getMatrixCplIea,
	updateMatrixCplIea,
} from "../service/ModelKonstruksi/Matrix/MatrixCplIeaService";
...
export const useMatriksCplIeaData = () => {
	...
	useEffect(() => {
		const fetchMatriksCplIea = async (prodiId = null) => {
			setLoading(true);
			try {
				const data = await getMatrixCplIea(prodiId);
		...
		fetchMatriksCplIea(selectedProdiId);
	}, [selectedProdiId]);
