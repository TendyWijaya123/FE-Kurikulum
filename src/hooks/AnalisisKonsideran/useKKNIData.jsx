...
export const useKKNIData = () => {
	...
	const fetchKkni = async (prodiId = null) => {
		setLoading(true);
		try {
			const data = await getKkni(prodiId);
			setKkni(data.kkni);
	...
	const handleautocpl = async () => {
		...
		try {
			...
			message.success(
				"berhasil membuat rancangan cpl berdasarkan analisis konsideran"
			);
