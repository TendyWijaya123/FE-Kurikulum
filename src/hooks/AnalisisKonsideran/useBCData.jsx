...
export const useBCData = () => {
        ...
        if (benchKurikulums.length > 0) {
            setDataSource(
                benchKurikulums.map((item, index) => ({
                    ...
                    cpl: item.bk_cpls?.length
                        ? item.bk_cpls.map((k) => k.cpl)
                        : [],
                    ppm: item.bk_ppms?.length
                        ? item.bk_ppms.map((k) => k.ppm)
                        : [],
                }))
            );
        } else {
    ...
    const handleAddRow = () => {
        ...
        const existingKeys = dataSource.map(item => parseInt(item.key.replace('benchKurikulums', ''))).sort((a, b) => a - b);
